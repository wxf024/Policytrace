...
function trObj(obj,key){const v=obj?.[key+'_i18n'];return v?.[currentLang]||obj?.[key]||''}
...
async function init(){
 ...
 <section class="section-block nav-section" id="term-record" data-section="term-record"><div class="section-head"><div><div class="section-kicker">${pt('master')}</div><h2>${pt('whatHappened')} ${adminLabel(a)}</h2></div><a class="big-link" href="index.html?pm=${encodeURIComponent(a.name)}#timeline">${pt('openFull')}</a></div><div class="term-legend"><strong>${pt('oneStream')}</strong> ${pt('legend')}</div><div class="profile-event-grid">${timeline.map(e=>`<article class="profile-event ${e.tone==='negative'?'profile-event-negative':''}"><div class="record-meta"><span class="pill record-type-pill">${typeOf(e)}</span><span class="pill">${e.date}</span>${e.category?`<span class="pill">${trObj(e,'category')}</span>`:''}${e.migration_relevance?`<span class="pill migration-pill">${trObj(e,'migration_relevance')}</span>`:''}</div><h3>${trObj(e,'title')}</h3><p>${trObj(e,'summary')}</p><div class="profile-relation"><span>${e.scope==='legacy'?'Legacy relation':'Relationship'}</span><strong>${trObj(e,'relationship')||trObj(e,'relation')||e.relationship||e.relation||''}</strong></div><div class="news-links">${(e.sources||[]).map(s=>`<a href="${s.url}" target="_blank" rel="noopener">${s.label} ↗</a>`).join('')}</div></article>`).join('')||'<p class="muted">No records yet.</p>'}</div></section>
 <section class="section-block nav-section" id="after-office" data-section="after-office"><div class="section-head"><div><div class="section-kicker">${pt('afterKicker')}</div><h2>${currentLang==='zh'?adminLabel(a)+' 离开唐宁街后做了什么':'What '+a.name+' did after leaving Downing Street'}</h2></div><p>Roles and activities are listed as documented facts. A commercial or institutional connection is not itself evidence of improper benefit.</p></div><div class="after-office-list">${a.current?'<article class="after-office-current"><span>CURRENT PM</span><h3>Still in office</h3><p>Post-office tracking begins after this administration ends.</p></article>':(post.map(x=>`<article class="after-office-card"><div class="after-office-date">${x.date}</div><div><div class="record-meta"><span class="pill">${x.status==='current'?'CURRENT / LATEST':'HISTORICAL'}</span>${x.tags.map(t=>`<span class="pill">${t}</span>`).join('')}</div><h3>${trObj(x,'title')}</h3><p>${trObj(x,'summary')}</p><div class="current-role"><span>Latest known status</span><strong>${trObj(x,'current')}</strong></div><div class="news-links">${x.sources.map(s=>`<a href="${s.url}" target="_blank" rel="noopener">${s.label} ↗</a>`).join('')}</div></div></article>`).join('')||'<p class="muted">No post-office record entered yet.</p>')}</div></section>
 ${promises.length?`<section class="section-block nav-section" id="profile-promises" data-section="profile-promises"><div class="section-head"><div><div class="section-kicker">${pt('accountability')}</div><h2>Promise vs result</h2></div></div><div class="promise-grid">${promises.map(p=>`<article class="promise-card"><div class="promise-admin">${p.date}</div><h3>${trObj(p,'promise')}</h3><div class="versus"><div><span>TARGET</span><strong>${trObj(p,'target')}</strong></div><b>VS</b><div><span>RESULT</span><strong>${trObj(p,'result')}</strong></div></div><p>${trObj(p,'context')}</p>${p.sources.map(s=>`<a href="${s.url}" target="_blank" rel="noopener">${s.label} ↗</a>`).join('')}</article>`).join('')}</div></section>`:''}`;
